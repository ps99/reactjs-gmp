...
const Main = () => {
    ...
    return (
      <li key={i} className="navigation-filter__item">
        <button
          ...
          value={item}
          onClick={(e) => setFilterState(e.currentTarget.value)}></button>
      </li>
    )
  });
  ...
  const naviSorter = sorts.map((item:string, i:number) => {
    ...
    return (
      <button
        ...
        className={filterState === item ? 'active' : undefined}
        value={item}
        onClick={(e) => setSortState(e.currentTarget.value)}></button>
    )
  });
  ...
  return (
    <main id="main" className="main"
      data-sort={sortState} data-filter={filterState}>
      <nav className="main-navigation">
        ...
        <details className='navigation-sorter'>
          <summary className='navigation-sorter-label'
            ><span>Sort by</span>{sortState}<i></i></summary>
          <div className="navigation-sorter-list">
            {naviSorter}
          </div>
        </details>
      </nav>
